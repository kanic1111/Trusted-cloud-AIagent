...
class TrustedCloudVMBuilder:
    ...
    def __init__(self, token):
        ...
        self.url = 'https://api.trusted-cloud.nchc.org.tw/'
...
################################################################################################################
    ...
    def list_image(self):
        ...
        url = self.url + "iam/api/v1/projects"
        response = requests.get(url, headers=self.headers)
    ...
    def list_module(self, module_name, module_category_id) -> dict:
        ...
        # 家燕測試機
        # url = self.aps_api_url + 'module-category/' + module_category_id + '/module-categories?limit=-1'
        response = requests.get(url, headers=self.headers)
