...
class VMResources:
    ...
    def get_available_options(self):
        return {
            "image_id": list(self.img_dict.keys()),
            "flavor_id": list(self.fla_dict.keys()),
            "network_id": list(self.net_dict.keys()),
            "security_group_id": list(self.sec_dict.keys()),
        }
...
def make_vm_param_setters(wrapper):
    ...
    def show_parameters():
        ...
        處理缺失參數的重複問答，可測試prompt等等修正測試
        '''
        print('---------------------------')
        print('判斷收集參數用')
        print('---------------------------')
        
        ctx = wrapper.context or {}
        
        missing_params = [k for k, v in ctx.items() if not v]
        
        if missing_params:
            return {
                "status": "incomplete",  # 加這個
                "message": f"⚠️ 尚有參數未設定完整，缺少：{', '.join(missing_params)}",
                "parameters": ctx
            }
...
class VMAgentManager:
    ...
    def __init__(self, token: str):
        ...
        self.temperature = initializer.parser['temperature']
...
def handle_vm_creation_flow(manager, user_input: str, session_store):
    ...
    if params_complete:
        if "確定建立" in user_input:
            result = manager.create_vm_from_context(confirmation="確定建立")
            if result["success"]:
            ...
            return False, "⚠️ 請輸入「確定建立」以開始建立 VM。"
    else:
